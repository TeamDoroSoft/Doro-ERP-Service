 ...
 * Transaction들을 시작하기 전에 딱 한 번만 계산해 재시도 사이에 절대 바뀌지 않는다 — 그래야 "같은 Category
 * 안에서 동시 요청 중 하나만 성공"이 재시도 여부와 무관하게 유지된다. 같은 Category 안에서 동시에 마지막
 * 순서를 다투면 실제 MySQL/InnoDB 동작은 두 갈래로 나타난다 — (1) uk_product_display_order UNIQUE
 ...
 * 충돌이 아니라 일시적인 잠금 스케줄링 문제이므로(서로 다른 Category 사이에서도 발생할 수 있다), 같은
 ...
 * 않았을 수 있어 짧게 재시도하며 기다린다), 그렇지 않으면 위와 같이 처리한다. uk_product_idempotency_key
 * UNIQUE 위반은 재조회 결과가 다른 내용이면 IdempotencyKeyReusedException으로 수렴시킨다.
 */
@Service
public class CreateProductService {
    ...
    public Product create(CreateProductCommand command, AuditContext auditContext) {
        ...
        Product existing = findExisting(command.idempotencyKey(), requestHash);
        if (existing != null) {
            return existing;
        }
        ...
        UUID catalogId =
                catalogRevisionRepository
                        ...
                        .catalogId();
        int nextDisplayOrder = productRepository.nextDisplayOrder(command.categoryId());

        for (int attempt = 1; ; attempt++) {
            try {
                return requiresNewTransaction.execute(
                        status -> createAndRecord(command, requestHash, catalogId, nextDisplayOrder, auditContext));
            } catch (CannotAcquireLockException ex) {
                if (attempt >= MAX_DEADLOCK_RETRY_ATTEMPTS) {
                    Product winner = reconcileIdempotentWinner(command.idempotencyKey(), requestHash);
                    if (winner != null) {
                        return winner;
                    }
                    throw new OptimisticLockingFailureException(
                            "반복된 Deadlock으로 Product 생성에 실패했습니다. categoryId=" + command.categoryId(), ex);
                }
                ...
                if (DisplayOrderConflictSupport.matchesConstraint(ex, "uk_product_display_order")) {
                    throw new OptimisticLockingFailureException(
    ...
    private Product findExisting(String idempotencyKey, String requestHash) {
        if (idempotencyKey == null) {
            return null;
        }
        Product existing = productRepository.findByIdempotencyKey(idempotencyKey).orElse(null);
        if (existing == null) {
            return null;
        }
    ...
    /**
     * uk_product_idempotency_key 위반으로 삽입이 실패했을 가능성을 확인한다. Idempotency-Key가 없는
     * 요청이거나, 재조회해도 대상이 없다면(=다른 원인의 무결성 위반) null을 반환해 원래 예외를 그대로
     * 전파하게 한다.
     */
    private Product reconcileIdempotentWinner(String idempotencyKey, String requestHash) {
        if (idempotencyKey == null) {
            return null;
        }
        Product winner =
                IdempotentWinnerLookup.await(() -> productRepository.findByIdempotencyKey(idempotencyKey).orElse(null));
        if (winner == null) {
            return null;
        }
    ...
    private Product createAndRecord(

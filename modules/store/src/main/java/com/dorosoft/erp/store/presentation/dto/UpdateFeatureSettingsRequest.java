...
public record UpdateFeatureSettingsRequest(
    ...
    private Map<FeatureCode, Boolean> parseCustomerFeatures(List<ProblemFieldError> errors) {
        Map<FeatureCode, Boolean> result = new EnumMap<>(FeatureCode.class);
        if (customerFeatures == null) {
            errors.add(new ProblemFieldError("customerFeatures", "REQUIRED"));
            return result;
        }
        customerFeatures.forEach((value, enabled) -> {
            ...
            try {
                ...
                if (enabled == null) {
                    errors.add(new ProblemFieldError(field, "REQUIRED"));
                } else {
                    result.put(code, enabled);
                }
            } catch (IllegalArgumentException | NullPointerException exception) {
                errors.add(new ProblemFieldError(field, "INVALID_SETTING_CODE"));
            }
        });
        return result;
    }

    private Map<NotificationEventCode, Boolean> parseNotificationEvents(List<ProblemFieldError> errors) {
        Map<NotificationEventCode, Boolean> result = new EnumMap<>(NotificationEventCode.class);
        if (notificationEvents == null) {
            errors.add(new ProblemFieldError("notificationEvents", "REQUIRED"));
            return result;
        }

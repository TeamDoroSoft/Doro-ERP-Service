...
class StoreSettingsQueryIntegrationTest {
    ...
    void returnsCompleteSettings() throws Exception {
        ...
        mockMvc.perform(get("/api/v1/store-settings").headers(actorHeaders("store.settings.read")))
    ...
    void missingPermissionReturns403() throws Exception {
        mockMvc.perform(get("/api/v1/store-settings").headers(actorHeaders("store.settings.update")))

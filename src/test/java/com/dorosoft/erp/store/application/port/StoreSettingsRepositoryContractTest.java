...
@DisplayName("StoreSettingsRepository Port 구조 제약(SR-STORE-007)")
class StoreSettingsRepositoryContractTest {

    private static final String RULE =
            "SR-STORE-007: 배포당 DataSource 1개가 이미 해당 업체 Schema를 가리키므로"
                    + " Repository Port는 업체 ID·Schema명을 파라미터로 받으면 안 된다. 위반: ";

    private static List<Method> declaredMethods() {
        return Arrays.stream(StoreSettingsRepository.class.getDeclaredMethods())
    ...
    @Test
    @DisplayName("반환 타입으로도 업체 ID를 노출하지 않는다")
    void noMethodReturnsTenantIdentifier() {
        for (Method method : declaredMethods()) {
            assertThat(method.getReturnType())
                    .as(RULE + method.getName() + " 반환 타입")
                    .isNotEqualTo(UUID.class)
                    .isNotEqualTo(String.class);
        }
    }
    ...
    void signaturesMatchContract() throws NoSuchMethodException {
        Method findCurrent = StoreSettingsRepository.class.getDeclaredMethod("findCurrent");
        Method exists = StoreSettingsRepository.class.getDeclaredMethod("exists");
        Method save =
                StoreSettingsRepository.class.getDeclaredMethod(
                        "save", com.dorosoft.erp.store.domain.settings.StoreSettings.class);
        ...
        assertThat(save.getReturnType())
                .isEqualTo(com.dorosoft.erp.store.domain.settings.StoreSettings.class);
    }
}
